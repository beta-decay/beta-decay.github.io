...
for i in range(len(county_list)):
    ...
    testing_episodes = county["Testing episodes (new)"]

    if i == 0:
        avg_dates = np.array(date[:])
        avg_sum_cases = np.array(new_cases[:])
        avg_sum_tests = np.array(testing_episodes[:])
    else:
        # Sum all counties
        avg_sum_cases += np.array(new_cases[:])
        avg_sum_tests += np.array(testing_episodes[:])
        
moving_average = np.zeros(len(avg_sum_cases)-7)

# Calculate 7-day moving average
for j in range(7,len(avg_sum_cases)):
    temp_arr = avg_sum_cases[j-7:j]/avg_sum_tests[j-7:j]*100
    #temp_arr[temp_arr!=temp_arr]=0
    moving_average[j-7] = sum(temp_arr)/7
...
plt.tight_layout()
plt.savefig("covid_n_wales_combined_tests.png",dpi=1000)
